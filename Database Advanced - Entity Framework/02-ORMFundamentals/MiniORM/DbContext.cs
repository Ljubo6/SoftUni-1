...
namespace MiniORM
{
    public abstract class DbContext
    {
        ...
        public void SaveChanges()
        {
            ...
            {
                using (var transation = this.connection.StartTransaction())
                {
        ...
        private void MapRealations<TEntity>(DbSet<TEntity> dbSet) where TEntity : class, new()
        {
        ...
        private void MapNavigationProperties<TEntity>(DbSet<TEntity> dbSet) where TEntity : class, new()
        {
            ...
            {
                var navigationPropName = fk.GetCustomAttribute<ForeignKeyAttribute>().Name;
                var navigationProperty = entityType.GetProperty(navigationPropName);
                ...
                foreach (var entity in dbSet)
                {
                    var fkValue = fk.GetValue(entity);

                    var navigationPropValue = ((IEnumerable<object>)navigationDbSet)
                        .First(currentProp => navigationPrimaryKey.GetValue(currentProp).Equals(fkValue));

                    navigationProperty.SetValue(entity, navigationPropValue);
                }
            }
        }
